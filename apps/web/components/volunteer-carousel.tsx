...
export function VolunteerCarousel() {
  ...
  const { pause, resume, goToPrevious, goToNext, goToIndex, currentIndex } = useCarouselAutoPlay({
    totalSlides: volunteers.length,
    onIndexChange: () => {},
  });

  useEffect(() => {
    goToIndex(Math.floor(Math.random() * volunteers.length));
    setReady(true);
  }, []);
  ...
  return (
    <div
      className={`w-full mb-16 px-4 py-8 transition-opacity duration-500 ${ready ? "opacity-100" : "opacity-0"}`}
      onMouseEnter={pause}
      onMouseLeave={resume}
    >
        ...
        <div className="relative flex items-center justify-center gap-4">
          <button
            ...
            style={{
              ...
              color: roleColors[volunteer.roles[0] ?? "logistics"]?.text || "text-blue-400",
            }}
...
function CarouselVolunteerCard({ volunteer }: { volunteer: Volunteer }) {
  ...
  const gradientColors = (roleColor ? colorMap[roleColor.text] : null) ?? colorMap["text-blue-400"];
